...
class TestSchemas(object):
	...
	def test_crontab_params(self):
		"""
		python -m pytest tests/test_schemas_new.py::TestSchemas::test_crontab_params
		"""

		with pytest.raises(exceptions.ValidationError, match="{'expression': 'This field is required.'}"):
			schemas.crontab_params.validate({})

		with pytest.raises(exceptions.ValidationError, match='Must be an object.'):
			schemas.crontab_params.validate({'foo'})

		with pytest.raises(exceptions.ValidationError, match="{'expression': 'This field is required.', 'foo': 'Invalid property name.'}"):
	...
	def test_job(self):
		"""
		python -m pytest tests/test_schemas_new.py::TestSchemas::test_job
		"""

		with pytest.raises(exceptions.ValidationError, match="{'trigger': 'This field is required.', 'task': 'This field is required.'}"):
			schemas.Job({})

		with pytest.raises(exceptions.ValidationError, match="{'task': 'This field is required.', 'trigger': 'Must match one of the union types.'}"):
	...
	def test_schema(self):
		"""
		python -m pytest tests/test_schemas_new.py::TestSchemas::test_schema
		"""
	...
	def test_schemas(self):
		"""
		python -m pytest tests/test_schemas_new.py::TestSchemas::test_schemas
		"""
			...
			with open('{}/{}'.format(path, file)) as f:
				...
				if 'job-fail' in file:
					...
					with pytest.raises((exceptions.ValidationError, KeyError)):
						schemas.Job.validate(j)

				elif 'job-pass' in file:
					assert schemas.Job.validate(j)
